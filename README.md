...
It is generated by [syft](https://github.com/anchore/syft) (pinned by digest) in a build stage that never reaches the final image, and it lists only *installed* packages: the declared ones in `uv.lock` - dev dependencies that are not in the image - are deliberately left out, because they would show up as vulnerabilities that cannot affect you.

Feed it to a scanner:

```bash
grype sbom:sbom.json --vex security/kronoterm2mqtt.openvex.json
trivy sbom sbom.json --vex security/kronoterm2mqtt.openvex.json
```

The `--vex` file is this project's analysis of findings that cannot be fixed and do not apply here - currently one: a denial of service in CPython's `html.parser`, which this application never uses, fixed only in Python 3.15. See [`security/README.md`](security/README.md) for how statements are written and, more importantly, when they must not be.

Expect the numbers to differ between tools, and between the SBOM and the image itself. Numbers move as advisories are published, which is why the published image is rebuilt weekly and scanned again rather than measured once. At the time of writing `trivy image` reported 0 HIGH/CRITICAL and `grype sbom:` reported 1 - CVE-2026-15308, a denial of service in CPython's `html.parser`, which this app never uses and which is fixed only in Python 3.15. Note also that `trivy sbom` currently matches far less than `trivy image` does on the same build. Treat the SBOM as the record of what is inside, and scan the image directly when you want Trivy's verdict:

```bash
trivy image kronoterm2mqtt:local
```

The build also stamps the usual OCI labels, so a running container can be traced back to a commit:

```bash
docker image inspect kronoterm2mqtt:local --format '{{json .Config.Labels}}'
```

Pass the values in when building, otherwise they read `unknown`:

```bash
VCS_REF=$(git rev-parse --short HEAD) \
BUILD_DATE=$(date -u +%Y-%m-%dT%H:%M:%SZ) \
APP_VERSION=$(python3 -c "import kronoterm2mqtt; print(kronoterm2mqtt.__version__)") \
docker compose build
```

[`security/README.md`](security/README.md) maps these to the CIS Docker Benchmark controls they satisfy, with a command to check each one, and says plainly what is *not* claimed (FIPS, STIG, a patching SLA).

Two things are left to the host, because they are daemon-wide rather than per-container: enabling user namespace remapping (`"userns-remap": "default"` in `/etc/docker/daemon.json`), and turning on the containerd image store if you want BuildKit attestations (`docker buildx build --sbom=true --provenance=mode=max`) - with the default docker driver those are refused, which is why the SBOM below is a file in the image instead.
...
* [v0.1.18](https://github.com/kosl/kronoterm2mqtt/compare/v0.1.17...v0.1.18)
  ...
  * 2026-05-26 - Fix missing error handlers in dev CLI
  * 2026-04-28 - Update envirionment
...
* [v0.1.17](https://github.com/kosl/kronoterm2mqtt/compare/v0.1.16...v0.1.17)
  ...
  * 2026-03-30 - Update github tests
  * 2026-03-30 - Fix assert_in cli test
  * 2026-03-30 - Assert in is now in cli_base
